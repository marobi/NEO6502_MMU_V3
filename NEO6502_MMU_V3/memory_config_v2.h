...
enum MemoryLayoutClass {
  MEMORY_LAYOUT_SYSTEM = 0,
  MEMORY_LAYOUT_USER = 1
};
...
enum MemoryIniErrorCode {
  ...
  MEMORY_INI_INVALID_LAYOUT_CLASS,
  MEMORY_INI_MODEL_NOT_FOUND,
  MEMORY_INI_LAYOUT_NOT_FOUND,
  MEMORY_INI_LAYOUT_MODEL_MISMATCH,
...
struct MemoryLayout {
  ...
  char name[MAX_MEMORY_NAME];
  MemoryLayoutClass layout_class;
...
struct MemoryConfig {
  ...
  char default_system_layout[MAX_MEMORY_NAME];
  char default_user_layout[MAX_MEMORY_NAME];
